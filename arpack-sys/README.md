...
64-bit-pointer-width targets only. 32-bit targets fail at compile time with `compile_error!` because the bindings do not bound workspace-size arithmetic against the smaller `usize` / `isize::MAX` envelope. Open an issue if 32-bit support is needed.
